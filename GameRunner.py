...
class GameRunner:
    ...
    def __init__(self):
        ...
        # do the handling
        self._button = Button(14, "button1", buttonhandler=None)
        self._timer = SoftwareTimer(None)
        self._displayTop = LCDDisplay(sda=0, scl=1, i2cid=0)
        self._gameControl = ColorMatchController()
    ...
    def stateEntered(self, state, event):
        ...
        if state == 0:
            self._displayTop.reset()
            self._displayTop.showText(f'Press Black')
            self._displayTop.showText(f'To Start',row=1)
        elif state == 1:
            self._gameControl.game()
